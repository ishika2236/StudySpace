...
function addTask() {
    ...
    displayTasks();
    updateBudgetInfo();
    updateAnalytics(); // New function to update analytics
    clearTaskForm();
}
...
function updateTaskStatus(index, status) {
    ...
    if (status === 'Completed') {
        ...
        // Set actual duration if not already set
        if (projectData.tasks[index].actualDuration === 0) {
            const startDate = new Date(projectData.tasks[index].startDate);
            const endDate = new Date();
            const diffTime = Math.abs(endDate - startDate);
            const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24)); 
            projectData.tasks[index].actualDuration = diffDays * 8; // Assuming 8 work hours per day
        }
    }
    displayTasks();
    updateAnalytics();
...
function calculateMemberContributions() {
    const contributions = {};
    ...
    document.getElementById('totalBudget').addEventListener('change', function() {
        ...
        projectData.budget = projectData.budget || {};
        projectData.budget.total = parseFloat(this.value) || 0;
        localStorage.setItem('projectData', JSON.stringify(projectData));
        updateAnalytics();
        // Refresh the charts or reload the page
    });